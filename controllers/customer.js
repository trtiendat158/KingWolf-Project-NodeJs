...
router.post('/signup', async function (req, res) {
  ...
  if (dbCust) {
    MyUtil.showAlertAndRedirect(res, 'Username or Email is already used, please choose another!', './signup');
  } else {
    ...
    if (newID) {
      ...
      if (result) {
        MyUtil.showAlertAndRedirect(res, 'CHECK YOUR EMAIL!', './login');
      } else {
        MyUtil.showAlertAndRedirect(res, 'WE CANNOT EMAIL YOU, PLEASE TRY AGAIN!', './signup');
      }
    } else {
      MyUtil.showAlertAndRedirect(res, 'YOUR INFO CANNOT BE SAVED, PLEASE TRY AGAIN!', './signup');
    }
  }
});
...
router.post('/login', async function (req, res) {
  ...
  if (customer && customer.active == 1) {
    ...
    MyUtil.showAlertAndRedirect(res, 'Wrong login info, please try again!', './login');
  }
});
...
router.post('/myprofile', async function (req, res) {
  ...
  if (curCust) {
    ...
    if (result) {
      ...
      MyUtil.showAlertAndRedirect(res, 'Success!', './home');
    }
  }
  MyUtil.showAlertAndRedirect(res, 'We cannot update your info, please try again!', './myprofile');
});
// myorders
...
router.get('/checkout', async function (req, res) {
  if (req.session.customer) {
    ...
    if (result) {
      ...
      MyUtil.showAlertAndRedirect(res, 'We cannot handling your order, please try again!', './mycart');
    }
  } else {
    res.redirect('./login');
  }
